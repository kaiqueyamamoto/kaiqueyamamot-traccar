<div class="group" data-id="{{ $item->id }}">
    <div class="group-heading">
        <div class="group-title {{ $item->completed_at ? 'collapsed' : '' }}" data-toggle="collapse" data-target="#checklist-{{ $item->id }}" aria-expanded="false" aria-controls="checklist-{{ $item->id }}">
    ...
    <div id="checklist-{{ $item->id }}" class="group-collapse collapse {{ $item->completed_at ? '' : 'in' }}" aria-expanded="false">
            ...
            <table class="table table-list table-condensed">
                ...
                <tbody>
                    @foreach ($item->rows as $row)
                        @include('Frontend.Checklist.partials.prestart_row')
                    @endforeach
                </tbody>
            </table>
            @if($item->notes)
