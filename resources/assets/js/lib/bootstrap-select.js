    ...
    Selectpicker.prototype = {
        ...
        createDropdown: function () {
            ...
            var drop =
                ...
                (this.options.icon ? '<span class="'+this.options.icon+' pull-left"></span>' : '') +
        ...
        checkSelectedGroups: function (optgroup) {
            if ( ! this.multiple)
                return;

            var $_menu = this.$menuInner,
                _selector = 'li.dropdown-header[data-optgroup]';

            if (optgroup)
                _selector = 'li.dropdown-header[data-optgroup="'+optgroup+'"]';

            $_menu.find(_selector).each(function(){
                var input = $(this).find('input'),
                    length = $_menu.find("li[data-original-index][data-optgroup=" + $(this).data('optgroup') + "]:not(.selected)")
                        .not('.divider, .dropdown-header, .disabled, .hidden').length,
                    status = length == 0;
        ...
        clickListener: function () {
            ...
            //this.$menuInner.off('change', 'li.dropdown-header input');
            this.$menuInner.on('change', 'li.dropdown-header input', function () {
                ...
                var opt_group   = $(this).closest('.dropdown-header').data("optgroup"),
                    $options    = that.$element.find('option'),
                    $lisVisible = that.$lis.not('.divider, .dropdown-header, .disabled, .hidden').filter("[data-optgroup=" + opt_group + "]"),
                    selectedOptions = [],
                    status = $(this).prop('checked');

                for (var i = 0; i < $lisVisible.length; i++) {
